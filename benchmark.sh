...
circuit_list=("sha256" "cubic" "exponentiate")
curve_list=("bn254" "bls12_377" "bls12_381" "bw6_761")
GPU_Acc=(false true)
GPU_ids=(0)
n=3
        ...
        for acc in "${GPU_Acc[@]}"; do
            if [ "$acc" == true ]; then
                for gpu in "${GPU_ids[@]}"; do
                    ...
                    go run -tags=icicle main.go -curve "$curve" -circuit "$circuit" -GPU_Acc -n "$n"
                done
            else
                go run -tags=icicle main.go -curve "$curve" -circuit="$circuit" -n "$n"
            fi
        done
    done
done
